    ...
    size_t bytes)
{
    ...
    size_t offset = OFFSETOF(UvisorAllocatorInternal, table.page_origins) + sizeof(((UvisorPageTable) {0}).page_origins);
    ...
    size_t maximum_malloc_size)
{
    ...
    if (allocator == NULL) {
        DPRINTF("uvisor_allocator_create_with_pages: UvisorAllocatorInternal failed to be allocated!\n\n");
        return NULL;
    }
